...
class DatabaseService {
  ...
  Future<PeerJob?> getPeerJobById(String jobId) async {
    try {
      final response = await _client
          .from('peer_jobs')
          .select('*, peer_job_skills(skill_id, skills(name)), users!user_id(id, full_name, profile_picture)')
          .eq('id', jobId)
          .single();
  ...
  Future<List<Transaction>> getTransactions(String walletId, {int limit = 20}) async {
    try {
      ...
      return response.map<Transaction>((tx) => Transaction.fromJson(tx)).toList();
    } catch (e) {
  ...
  Future<List<PaymentMethod>> getUserPaymentMethods(String userId) async {
    try {
      ...
      return response.map<PaymentMethod>((pm) => PaymentMethod.fromJson(pm)).toList();
    } catch (e) {
